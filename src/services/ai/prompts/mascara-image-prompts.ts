...
${brandModifier}
${additionalModifiers}
professional e-commerce detail page photography, high-end mascara advertising, 8K resolution, dramatic confident mood, black and pink accent lighting
`.trim().replace(/\n+/g, ', ').replace(/,\s*,/g, ',');

  // 네거티브 프롬프트
  if (includeNegative) {
    prompt += ` --negative low quality, blurry, noisy, amateur, cartoon, anime, illustration, watermark, text, letters, numbers, words, Korean text, English text, any characters, logo text, brand name text, labels, captions, signatures, hand-written, distorted product, wrong proportions, clumpy lashes, spider lashes, smudged makeup, raccoon eyes, unnatural lash texture`;
  }
