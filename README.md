...
> 상품 상세페이지 제작 시간을 1시간 → 10분으로 단축하는
> AI 기반 마케팅 콘텐츠 자동화 플랫폼

## 기획 배경
브랜드마다 톤앤매너가 다른데, 기존 AI 생성 도구는 
획일적인 결과물을 냈습니다. 디자이너·카피라이터 리소스 없이도
브랜드 정체성을 반영한 상세페이지를 자동 생성하는 것을 목표로
기획했습니다.

## 담당 역할 (김지원)
- AI 이미지 생성 파이프라인 설계 및 프롬프트 엔지니어링
- 13개 에이전트 오케스트레이션 구조 설계
- 에이전트 간 응답 불일치 이슈 추적 및 해결 (GitHub Issues)
- PRD · TRD 작성 및 서비스 기획 전반 참여

## 핵심 의사결정
- **RAG 기반 브랜드 분석 도입**: 브랜드 URL만 입력하면
  톤앤매너를 자동 추출해 생성에 반영. 수동 가이드 참조 제거.
- **2-Step 이미지 파이프라인**: Gemini 이미지 생성 시
  텍스트 오버레이가 깨지는 문제를 생성-렌더링 분리로 해결.
- **이중 확인 구조 설계**: 사용자가 실수로 생성을 확정하는
  것을 막기 위해 confirm 단계를 두 번 거치도록 UX 설계.
- **Fallback 정책**: 검색 결과 0건·API 오류 시 시스템이
  중단되지 않고 안내 메시지를 출력하도록 예외 흐름 설계.

## 산출물
- [PRD 바로가기](./Plan_prd_trd/PRD_TripleC_EN.md)
- [TRD 바로가기](./Plan_prd_trd/TRD_TripleC_EN.md)
...
[![E2E Tests](https://img.shields.io/badge/E2E_Tests-55_cases-4FC08D?logo=playwright&logoColor=white)](./tests/e2e)
