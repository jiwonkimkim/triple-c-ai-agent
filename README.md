...
```mermaid
  ...
  │   단계   │            예외            │         코드 위치         │           실제 동작           │
  ├──────────┼────────────────────────────┼───────────────────────────┼───────────────────────────────┤
  │ 정보수집 │ LLM API 실패               │ intake.ts LLM 호출 블록   │ 오류 메시지 → await_input     │
  │          │ (INTAKE/CLARIFIER)         │                           │                               │
  ├──────────┼────────────────────────────┼───────────────────────────┼───────────────────────────────┤
  │ 정보수집 │ /api/upload 실패           │ chat-input.tsx:124 →      │ toast 알림 후 재시도 가능     │
  │          │                            │ route.ts                  │                               │
  ├──────────┼────────────────────────────┼───────────────────────────┼───────────────────────────────┤
  │ 기획     │ LLM 플랜 생성 실패         │ planner.ts LLM 호출 블록  │ nextAction: error → Feedback  │
  │          │                            │                           │ 위임                          │
  ├──────────┼────────────────────────────┼───────────────────────────┼───────────────────────────────┤
  │ 생성     │ 중복 요청                  │ generator.ts:18           │ 기존 프로젝트 반환, 신규 생성 │
  │          │ (status=GENERATING)        │                           │  차단                         │
  ├──────────┼────────────────────────────┼───────────────────────────┼───────────────────────────────┤
  │ 생성     │ 필수 필드 누락             │ generator.ts:88 +         │ GEN_ERR → nextAction: error → │
  │          │                            │ types.ts:484              │  Feedback                     │
  ├──────────┼────────────────────────────┼───────────────────────────┼───────────────────────────────┤
  │ 생성     │ AI API 타임아웃/실패       │ generator.ts:243          │ GEN_ERR → Feedback으로 라우팅 │
  ├──────────┼────────────────────────────┼───────────────────────────┼───────────────────────────────┤
  │ 생성     │ DB 오류                    │ generator.ts:129, 273,    │ GEN_ERR → Feedback으로 라우팅 │
  │          │                            │ 334                       │                               │
  ├──────────┼────────────────────────────┼───────────────────────────┼───────────────────────────────┤
  ...
  1. 생성 확인 이중 구조: readyToGenerate=true 후 COORDINATOR가 한 번 더 확인 메시지를 보내야
  confirmedGenerate=true가 됨 → 확인 없이 바로 생성 진입하는 경로 테스트 필요
  2. 이미지 스킵 키워드 의존: route.ts:297 에서 '스킵/건너뛰/없어...' 키워드 매칭으로 스킵 처리 → 다른 표현
   시 waitingForImageUpload 상태에 갇힘
  3. SUGGESTER 순차 루프: 필드 완성 후 SUGGESTER가 다음 빈 필드로 계속 이동 → 특정 필드 스킵 불가 시 무한
  대기
